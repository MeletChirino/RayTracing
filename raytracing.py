...
def eclairage(obj, light, P):
    ...
    # TODO: Descriptiom de la fonction
    ...
    if pvect < 0:
        #f.write("HEREEEEEE!!!JLXHLUGLIFLG>KLHL:")
        pvect = 0
    # Coleur Total
    ct = cd + (ks * ls * ((pvect) ** (B / 4.0)))
    if not(pvect < 0):
        pass
        #f.write(F"\n|pvect = {pvect}\n|cd = {cd}\n|ct = {ct}\n|Ks * ls = {ks * ls}\n|tout = {(ks * ls * ((pvect) ** (B / 4)))}")
#     ct = list(ct)
#     liste = []
#     for i in ct:
#         if i < 0:
...
def trace_ray(ray):
    # TODO: Faire description du fonction
    ...
    if distance_vector == np.inf:
        ...
        # Intersection entre objet et ray
